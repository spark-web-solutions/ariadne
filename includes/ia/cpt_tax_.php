...
class cptTaxClass {
	function __construct($singular, $plural, array $posttypes, array $post_args = array(), array $tax_args = array(), $slug = '') {
		...
		$this->tax = new taxClass($singular, $plural, $posttypes, $tax_args, $slug);
        add_action('save_post', array($this, 'cpt_as_taxonomy'));
        add_action('before_delete_post', array($this, 'refresh_cpt_hierarchy'));
        add_action('deleted_post', array($this, 'delete_cpt_as_taxonomy'));
	}

	function cpt_as_taxonomy($post_id) {
    ...
    function delete_cpt_as_taxonomy($post_id) {

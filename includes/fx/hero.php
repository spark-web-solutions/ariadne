<?php
if (!function_exists('spark_get_hero_images')) {
	/**
	...
	function spark_get_hero_images($post = null, &$last_id = null) {
	    ...
	    if (false === ($images = get_transient($transient)) || !Spark_Transients::use_transients()) {
	        if ($post instanceof WP_Post) {
	            $large_hero_meta = spark_get_value_from_hierarchy('hero_image', $post->ID, $last_id);
	            if (!empty($large_hero_meta)) {
	                $large_image = wp_get_attachment_image_src($large_hero_meta, 'full');
	                $large_hero = $large_image[0];
	            }
	            if (empty($large_hero)) {
	            	$large_hero = spark_get_value_from_hierarchy('featured_image', $post->ID, $last_id);
	            }
	        } elseif ($post instanceof WP_Term) {
	        ...
	        if ($post instanceof WP_Post) {
	        	$medium_hero_meta = spark_get_value_from_hierarchy('hero_image_medium', $post->ID);
	        ...
	        if ($post instanceof WP_Post) {
	        	$small_hero_meta = spark_get_value_from_hierarchy('hero_image_small', $post->ID);
	        ...
	        if (Spark_Transients::use_transients()) {
	        	Spark_Transients::set($transient, $images);
	        }
	    }
	    unset($transient);

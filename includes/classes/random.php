...
abstract class Spark_Random {
     ...
     * @param integer $exclusion_period Optional. How long (in hours) returned items should be excluded from being selected again (default 48). Ignored if $refresh_rate is empty.
     * @return null
     */
    public function __construct($name, $args, $refresh_rate = 24, $exclusion_period = 48) {
        ...
        $exclusions = Spark_Transients::get($this->transient_prefix.'_exclusion');
    ...
    /**
     * Placeholder for loading data based on defined arguments. Must be overridden in child classes, and needs to populate $this->posts.
     * @return null
     */
    ...
    public function reset() {
        return Spark_Transients::delete($this->transient_prefix);
    }
}

/**
 * Implementation of Spark_Random which functions on children of the specified post
 */
class Spark_Random_Children extends Spark_Random {
    /**
     * Load all posts which are children of the specified post ID.
     * @return null
     */
    protected function load() {
        $this->posts = spark_get_children($this->args);
    }
}

/**
 * Implementation of Spark_Random which functions on any get_posts() compatible list of arguments
 */
class Spark_Random_Posts extends Spark_Random {
    /**
...
/**
 * Implementation of Spark_Random which functions on any get_users() compatible list of arguments
 */
class Spark_Random_Users extends Spark_Random {
    /**

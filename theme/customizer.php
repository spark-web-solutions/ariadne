...
function spark_theme_customizer(WP_Customize_Manager $wp_customize) {
    ...
    $wp_customize->add_setting(ns_.'logo_large', array(
            'default' => esc_url(get_template_directory_uri()).'/images/placeholders/logo-large.png',
    ...
    $wp_customize->add_setting(ns_.'logo_medium', array(
            'default' => esc_url(get_template_directory_uri()).'/images/placeholders/logo-medium.png',
            'sanitize_callback' => 'esc_url_raw',
            'type' => 'option',
    ));
    ...
    $wp_customize->add_setting(ns_.'logo_small', array(
            'default' => esc_url(get_template_directory_uri()).'/images/placeholders/logo-small.png',
            'sanitize_callback' => 'esc_url_raw',
            'type' => 'option',
    ));
    ...
    $wp_customize->add_setting(ns_.'logo_footer', array(
            'default' => esc_url(get_template_directory_uri()).'/images/placeholders/logo-footer.png',
            'sanitize_callback' => 'esc_url_raw',
            'type' => 'option',
    ));
    ...
    $patterns = spark_get_theme_mod(ns_.'patterns', 0);
    ...
    for ($i = 1; $i <= $colours; $i++) {
        ...
        $wp_customize->add_control(new WP_Customize_Color_Control($wp_customize, ns_.'colour'.$i, array(
                ...
                'description' => 'bg'.$i.', hbg'.$i.', text'.$i.', htext'.$i.', border'.$i.', hborder'.$i.'<br>spark_get_theme_mod(\''.ns_.'colour'.$i.'\');',
                'section' => ns_.'palette',
                'priority' => 10 + $i,
        )));
    }
    ...
    // Colour Scheme
    if (true == spark_get_theme_mod(ns_.'colour_scheme_check')) {
        ...
        $elements = spark_get_page_elements();
        $e = 1;
...
add_action('customize_save_after', 'spark_update_dynamic_styles');
function spark_update_dynamic_styles() {
    $result = false;
    $styles = spark_generate_dynamic_styles();
    ...
    if ($access_type === 'direct') {
        ...
        if (WP_Filesystem($creds)) {
            ...
            $result = $wp_filesystem->put_contents(get_stylesheet_directory().'/css/'.spark_get_dynamic_styles_filename(), $styles);
        }
    }

    // WP couldn't write to file automatically - we'll just do it directly rather than asking user for FTP details
    if (!$result) {
        $result = file_put_contents(get_stylesheet_directory().'/css/'.spark_get_dynamic_styles_filename(), $styles);
    }

    if ($result !== false) {
        Spark_Transients::delete('css');
    }
}

function spark_get_dynamic_styles_filename() {
...
function spark_generate_dynamic_styles() {
    ...
    $h_base = spark_get_theme_mod('h_base');
    $styles .= substr($heading_selectors, 0, -2).' {'.$h_base.'}'."\n";
    for ($i=1; $i <= 6; $i++) {
        ${'h'.$i} = spark_get_theme_mod(ns_.'h'.$i);
        if (!empty(${'h'.$i})) {
    ...
    for ($i = 1; $i <= $colour_count; $i++) {
        ${'colour'.$i} = spark_get_theme_mod(ns_.'colour'.$i);
    }
    ...
    $gradient = array();
    $gradient['start'] = 'rgba('.spark_convert_colour( spark_get_theme_mod( ns_.'gradient_start' ) ).',1)';
    $gradient['end'] = 'rgba('.spark_convert_colour( spark_get_theme_mod( ns_.'gradient_end' ) ).',1)';
    $gradient['start_percent'] = spark_get_theme_mod( ns_.'gradient_start_percent' );
    $gradient['end_percent'] = spark_get_theme_mod( ns_.'gradient_end_percent' );

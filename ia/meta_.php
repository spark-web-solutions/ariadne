...
class metaClass {
    ...
    function spark_metabox() {
        foreach ($this->posttypes as $post_type) {
            add_meta_box('meta_'.$this->slug.'_box', __( $this->label, '' ), array($this, 'spark_metabox_content'), $post_type, 'side', 'high');
        }
    }

    function spark_metabox_content( $post ) {
        if( !is_array( $meta_fields) ) $meta_fields = array();
        wp_nonce_field(plugin_basename( __FILE__ ), 'spark_metabox_content_nonce');

        foreach ($this->fields as $field) {
            array_push($meta_fields, $this->spark_new_field($field));
        }

        set_transient($this->slug.'_meta_fields', serialize($meta_fields), 3600);
    }

    function spark_metabox_save( $post_id ) {
        if (defined('DOING_AUTOSAVE') && DOING_AUTOSAVE) {
            return;
        }
        if (!wp_verify_nonce($_POST['spark_metabox_content_nonce'], plugin_basename(__FILE__))) {
            return;
        }

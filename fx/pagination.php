 ...
 * @return string
 */
function spark_p_link($i, $querystring = false, $title = '') {
    global $wp_query;
    $max_page = $wp_query->max_num_pages;
    if ($i == 1 || $title == 'first') {
        $rel = 'rel="first"';
    ...
    switch ($title) {
        ...
        case 'last':
            ...
            break;
        case 'previous':
            $readabletitle = $linktext = _x('‹ Previous', 'pagination previous page', THEME_TEXTDOMAIN);
            $rel = 'rel="prev"';
            break;
        case 'next':
            $readabletitle = $linktext = _x('Next ›', 'pagination next page', THEME_TEXTDOMAIN);
            $rel = 'rel="next"';
            break;
        default:
